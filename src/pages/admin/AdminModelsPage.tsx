import { Button, Card, Form, Input, InputNumber, Modal, Segmented, Select, Space, Switch, Table, Typography, message } from "antd";
import { AdminLayout } from "@/components/layout/AdminLayout";
import { SectionHeader } from "@/components/common/SectionHeader";
import { StatusTag } from "@/components/common/StatusTag";
import { useAnimeEntrance } from "@/hooks/useAnimeEntrance";
import { useCallback, useEffect, useState } from "react";
import type { ApiImageModel, ApiProvider, ApiVideoModel } from "@/api/types";
import {
...
export function AdminModelsPage() {
  ...
  const [modelType, setModelType] = useState<"video" | "image">("video");
  const [models, setModels] = useState<ApiVideoModel[]>([]);
  const [imageModels, setImageModels] = useState<ApiImageModel[]>([]);
  const [providers, setProviders] = useState<ApiProvider[]>([]);
  const [loading, setLoading] = useState(false);
  const [editing, setEditing] = useState<ApiVideoModel | ApiImageModel | null>(null);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [modelResult, imageModelResult, providerResult] = await Promise.all([listAdminModels(), listAdminImageModels(), listProviders()]);
      setModels(modelResult.list);
      setImageModels(imageModelResult.list);
  ...
  const openModal = (record?: ApiVideoModel | ApiImageModel) => {
    setEditing(record ?? null);
    if (modelType === "image") {
      const imageRecord = record as ApiImageModel | undefined;
      form.setFieldsValue(imageRecord ? {
        ...imageRecord,
        supported_input_types: imageRecord.supported_input_types,
        aspect_ratio_options: imageRecord.aspect_ratio_options.join(","),
        supported_sizes: imageRecord.supported_sizes.join(",")
      } : {
        status: "maintenance",
        price_per_image: 1,
        max_n: 1,
      ...
      const videoRecord = record as ApiVideoModel | undefined;
      form.setFieldsValue(videoRecord ? {
        ...videoRecord,
        duration_options: videoRecord.duration_options.join(","),
        supported_input_types: videoRecord.supported_input_types,
        aspect_ratio_options: videoRecord.aspect_ratio_options.join(","),
        supported_resolutions: videoRecord.supported_resolutions.join(",")
      } : {
        status: "maintenance",
  ...
  const handleSave = async () => {
    ...
    const payload = modelType === "image"
      ? {
        ...values,
        supported_input_types: values.supported_input_types,
        supported_input_combinations: values.supported_input_types?.includes("image") ? [["text"], ["text", "image"]] : [["text"]],
        aspect_ratio_options: splitValues(values.aspect_ratio_options),
        supported_sizes: splitValues(values.supported_sizes),
        input_asset_rules: { image: { max_count: 10, max_size_mb: 20, allowed_formats: ["jpg", "jpeg", "png", "webp", "heic", "avif"], input_method: "url" } },
        capabilities: { reference_image: values.supported_input_types?.includes("image") ?? false },
        upstream_default_params: { response_format: "url" },
    ...
    try {
      if (modelType === "image") {
        if (editing) {
          await updateAdminImageModel(editing.id, payload);
        } else {
    ...
    <AdminLayout>
            ...
            extra={
              <Space>
                <Segmented
                  value={modelType}
                  onChange={(value) => {
                    setModelType(value as "video" | "image");
                    setEditing(null);
                    form.resetFields();
                  }}
                  options={[
                ...
                />
                <Button type="primary" onClick={() => openModal()}>新增模型</Button>
              </Space>
            }
          />
        </Card>
          ...
          {modelType === "image" ? (
            <Table<ApiImageModel>
              ...
              columns={[
                ...
                { title: "价格", dataIndex: "price_per_image", key: "price_per_image", render: (value: number) => `${value} 积分 / 张` },
                { title: "最大张数", dataIndex: "max_n", key: "max_n" },
                {
                  title: "输入能力",
                    ...
                    <Space>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          await updateAdminImageModelStatus(record.id, next);
      ...
      >
        <Form form={form} layout="vertical">
          ...
          {modelType === "image" ? (
            <Space size={16} style={{ width: "100%" }} align="start">
              <Form.Item label="每张积分" name="price_per_image" rules={[{ required: true }]} style={{ flex: 1 }}>
                <InputNumber min={1} style={{ width: "100%" }} />
              </Form.Item>
              <Form.Item label="最大张数" name="max_n" rules={[{ required: true }]} style={{ flex: 1 }}>
          ...
          <Form.Item label={modelType === "image" ? "尺寸选项，逗号分隔" : "分辨率选项，逗号分隔"} name={modelType === "image" ? "supported_sizes" : "supported_resolutions"} rules={[{ required: true }]}>
            <Input placeholder={modelType === "image" ? "1024x1024,1536x1024,1024x1536,auto" : "1280x720,720x1280"} />
          </Form.Item>
          ...
          <Form.Item label="上游模型 ID" name="upstream_model_id">
            <Input placeholder={modelType === "image" ? "gpt-image-2 / nano-banana / nano-banana-pro" : "sora-2 / veo-3.1 / veo-3.1-fast"} />
          </Form.Item>
