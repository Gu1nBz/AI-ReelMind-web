...
| 字段 | 枚举值 | 说明 |
| --- | --- | --- |
| `model.status` | `available` `maintenance` `coming_soon` `disabled` | `disabled` 只在后台可见；用户端公共模型接口不返回停用模型 |
...
| `task.status` | `pending` `submitted` `processing` `succeeded` `failed` `refunding` `refunded` `timed_out` | 任务完整生命周期 |
...
| 页面 | 可见功能 | 需要接口 |
| --- | --- | --- |
| `/` 工作台 | 获取模型、提示词字段、价格试算、上传素材、创建任务、查看最近任务 | `GET /public/models` `GET /public/prompt-fields` `POST /public/generation/estimate` `POST /user/uploads` `POST /user/tasks` `GET /user/tasks` |
...
| `/redeem` 兑换 | 兑换码校验与兑换、最近兑换记录 | `POST /user/redeem-codes/redeem` `GET /user/redeem-codes/records` |
...
| `/admin/redeem-codes` | 列表、筛选、单个创建、批量生成、禁用、导出 CSV | `GET /admin/redeem-codes` `POST /admin/redeem-codes` `POST /admin/redeem-codes/batch` `PATCH /admin/redeem-codes/{id}` `GET /admin/redeem-code-exports` |
...
- 工作台不实现保存草稿接口，前端不保留草稿入口
- 用户端公共模型接口只返回 `available` `maintenance` `coming_soon`，不返回 `disabled`
...
当前 MVP 不实现保存草稿能力，前端也不保留草稿入口。后端不需要提供 `task-drafts` 相关接口，避免扩大联调范围。
...
### 10.2 `GET /api/v1/user/redeem-codes/records`
...
说明：  
不建议做物理删除。模型下线用 `disabled` 状态。`disabled` 模型不在用户端公共模型接口返回，但历史任务仍可查看。
...
- 请求字段建议：
  ...
  - `api_base_url`
  - `auth_type`
  - `credentials`，仅创建和更新时提交，响应中不返回明文
  - `update_mode`
  - `callback_secret`，仅创建和更新时提交，响应中不返回明文
...
### 14.6 `PATCH /api/v1/admin/prompt-field-orders`
...
### 15.6 `PATCH /api/v1/admin/package-orders`
...
### 16.7 `GET /api/v1/admin/redeem-code-exports`
...
第一版不提供 `retry-submit` 接口。任务创建后不能取消；失败或超时后走退款闭环。后续如果需要重试，建议创建新的任务记录，而不是复用原失败任务。
